...
contract Market {
    ...
    function getNftsForSale() public view returns (uint256[] memory) {
        uint256 totalSupply = nftContract.totalSupply();  // 获取NFT总数
        uint256[] memory nftsOnSale = new uint256[](totalSupply);  // 初始化一个数组

        uint256 index = 0;
        for (uint256 i = 0; i < totalSupply; i++) {
            uint256 tokenId = nftContract.tokenByIndex(i);  // 获取某个tokenId
            if (_isForSale[tokenId]) {
                nftsOnSale[index] = tokenId;  // 如果这个 NFT 在出售，将其添加到数组中
                index++;
            }
        }
